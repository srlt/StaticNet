...
**/
template<nat_t dim> class Vector {
private:
...
public:
    ...
    **/
    val_t* load(val_t* dump) {
        for (nat_t i = 0; i < input_dim; i++)
            weight.set(i, dump[i]);
        dump += input_dim * sizeof(val_t);
        bias = *dump;
        return dump + 1;
    }
    /** Save neuron data to dump.
     * @param dump Dump to write
     * @return New dump address
    **/
    val_t* save(val_t* dump) {
        for (nat_t i = 0; i < input_dim; i++)
            dump[i] = weight.get(i);
        dump += input_dim * sizeof(val_t);
        *dump = bias;
...
public:
    ...
    **/
    ...
    /** Save layer data to dump.
     * @param dump Dump to write
     * @return New dump address
    **/
    val_t* save(val_t* dump) {
        for (nat_t i = 0; i < output_dim; i++)
            dump = neurons[i].save(dump);
        return dump;
    }
    /** Print neuron weights to the given stream.
     * @param ostr Output stream
...
public:
    ...
    **/
    val_t* save(val_t* dump) {
        return layers.save(layer.save(dump));
    }
    /** Print neuron weights to the given stream.
     * @param ostr Output stream
    **/
